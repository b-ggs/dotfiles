# idk man this was generated by claude to fix an issue where global just targets
# don't get tab completion
# issue: https://github.com/casey/just/issues/2394
...
'--version[Print version]' \
    ...
    case $state in
        args)
            ...
            # Check if -g or --global-justfile was used
            local use_global=0
            for ((i = 1; i < $#words; i++ )) do
                if [[ ${words[i]} == "-g" || ${words[i]} == "--global-justfile" ]]; then
                    use_global=1
                    break
                fi
            done

            # Get commands based on whether global flag is used
            local cmds
            if (( use_global )); then
                cmds=(
                    ${(s: :)$(_call_program commands just -g --summary)}
                )
            else
                cmds=(
...
_just_commands() {
    ...
    # Get variables and commands based on whether global flag is used
    local variables
    local commands
    
    if (( use_global )); then
        variables=(
            ${(s: :)$(_call_program commands just -g --variables)}
        )
        commands=(
            ${${${(M)"${(f)$(_call_program commands just -g --list)}":#    *}/ ##/}/ ##/:Args: }
        )
    else
        variables=(
...
_just_variables() {
    ...
    # Get variables based on whether global flag is used
    local variables
    if (( use_global )); then
        variables=(
            ${(s: :)$(_call_program commands just -g --variables)}
