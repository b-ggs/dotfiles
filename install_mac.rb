...
def install_dependencies
  log "Installing dependencies from Homebrew..."
...
def install_cask_dependencies
  log "Installing dependencies from homebrew-cask..."
  cask_dependencies.each do |dependency|
    log "Installing #{dependency}..."
    if shell("brew cask list #{dependency}")[:success?]
      log "#{dependency} already exists. Skipping..."
    else
      if shell("brew cask install #{dependency}")
        log "Successfully installed #{dependency}."
      else
        log "There was a problem installing #{dependency}."
        exit 1
